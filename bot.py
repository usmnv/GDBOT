...
# Регистрация и поиск трек-кода
PHONE, ADMIN_CODE, TRACK_CODE = range(3)

# Админ: изменение курсов, доставки, управление заказами, рассылка
...
# Универсальный обмен валют (три шага)
EXCHANGE_SELECT_FROM, EXCHANGE_SELECT_TO, EXCHANGE_ENTER_AMOUNT = range(10, 13)

# ------------------------- Вспомогательные функции -------------------------
def get_main_keyboard(is_admin=False):
    """Главная клавиатура с новым пунктом 'Склады в Китае'."""
    keyboard = [
        ["👤 Личный кабинет"],
        ["💰 Курсы валют", "💱 Обмен валют"],
        ["🚚 Доставка"],
        ["🏭 Склады в Китае"],          # <-- НОВЫЙ ПУНКТ
        ["🔎 Поиск по трек-коду"],
        ["🆘 Поддержка"]
    ]
    if is_admin:
...
# ------------------------- Обработчики существующих команд -------------------------
...
async def personal_cabinet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Личный кабинет с кнопкой WebApp (мини-приложение)."""
    ...
    keyboard = [[
        InlineKeyboardButton(
         ...
         web_app={"url": f"https://usmnv.github.io/Gd-cargo/?code={user_data['customer_code']}"}
       )
    ]]
...
async def exchange_rates_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показ курсов валют (только просмотр)."""
...
async def handle_delivery_method(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показ деталей выбранного способа доставки."""
...
async def search_track_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Вход в диалог добавления трек-кода."""
...
async def handle_track_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка введённого трек-кода."""
    user_id = update.effective_user.id
...
# ------------------------- НОВЫЙ: Универсальный обмен валют -------------------------
...
async def exchange_select_from(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Шаг 2: после выбора исходной валюты – выбор целевой."""
...
async def exchange_select_to(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Шаг 3: выбор целевой валюты, запрос суммы."""
    text = update.message.text.strip()
    if text == "🔙 Назад":
...
async def exchange_enter_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Шаг 4: ввод суммы, расчёт и вывод результата."""
    text = update.message.text.strip()
    if text == "🔙 Назад":
...
# ------------------------- Отладка и вспомогательные команды -------------------------
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "👤 Личный кабинет":
        ...
        # Этот пункт обрабатывается ConversationHandler, здесь ничего не делаем
        pass
    elif text == "🚚 Доставка":
    ...
    # Автоматическое добавление трек-кода (если похоже на трек-номер)
...
def main():
    ...
    # УНИВЕРСАЛЬНЫЙ ОБМЕН ВАЛЮТ (новый)
    ...
    application.add_handler(conv_exchange)          # <-- новый обмен валют
    ...
    # Основной обработчик текстовых сообщений (должен быть последним)
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))

    logger.info("🤖 Golden Dragon Bot запущен...")
    logger.info("✅ Режимы: склады, универсальный обмен валют, мини-приложение")
